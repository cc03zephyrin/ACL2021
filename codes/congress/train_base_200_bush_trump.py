...
vs = 200.01# vamprior, adding masked labels, randomly masked
penalty = 1
...
flags.DEFINE_float('notmask', 0.75, 'non_masked label')
...
def iterative_training():
	...
	if not FLAGS.biased:
		...
		revdummies = [list(As_ids).index(i) for i in idsrev if i in As_ids]
		train_id= As_all[:, revdummies].sum(axis = -1)==1
		test_id = As_all[:, mskdummies].sum(axis = -1)==1 # no need ot filter swithchers
		...
		As_ids_rankfilt = [i for i in As_ids_ranked if i in au_c]
		tbn = int(len(As_ids_rankfilt) * FLAGS.notmask/2)


		print('start biased sampling')
		idsrev = np.array(As_ids_rankfilt[:tbn] + As_ids_rankfilt[-tbn:])
		idsmsk = np.array(As_ids_rankfilt[tbn:-tbn])
		mskdummies = [list(As_ids).index(i) for i in idsmsk if i in As_ids]
		revdummies = [list(As_ids).index(i) for i in idsrev if i in As_ids]
		train_id= As_all[:, revdummies].sum(axis = -1)==1
		test_id = As_all[:, mskdummies].sum(axis = -1)==1 # no need ot filter swithchers
	...
	# ids_ns = ids_all[train_id_wo_switchers]
	# ds_ns = ds_all[train_id_wo_switchers]
	# As_ns = As_all[train_id_wo_switchers]
	# thetas_ns = thetas_all[train_id_wo_switchers]
	# parties_ns = parties_all[train_id_wo_switchers]
	# dws_ns = dws_all[train_id_wo_switchers]


	ids, ids_val = ids_all[train_id], ids_all[test_id]
	ds, ds_val = ds_all[train_id], ds_all[test_id]
	As, As_val = As_all[train_id], As_all[test_id]
	thetas0, thetas_val = thetas_all[train_id], thetas_all[test_id]
	parties, parties_val = parties_all[train_id], parties_all[test_id]
