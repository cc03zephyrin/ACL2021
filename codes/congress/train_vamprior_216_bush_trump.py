...
vs = 216.02# vamprior, adding masked labels, randomly masked
penalty = 1
...
def predict_party_membership(s_pred, y, As):
	...
	y_pred = ave_slants>0
	p_acc = np.mean(y_pred == y_true)
	return ave_slants, p_acc
...
def train(trains, vals, tests, T0, T00,P0 = np.zeros([FLAGS.n_topics, FLAGS.latent_dim]), Zbar = None,  updateT = None, round = 0, do_val = False, writesummary = False):
	...
	with tf.Graph().as_default():
		...
		with tf.Session() as sess:
			...
			if (FLAGS.notmask == 0.05) or (FLAGS.notmask == 0.8):
...
def iterative_training():
	...
	# np.save(datapath + '/bush_trump_congress_ids_with_switcher', val_spokeid)
	...
	np.save(datapath+'/bush_trump_congress_parties_with_switcher', Party_id_valid)
	...
	for round in range(FLAGS.alter_rounds):
		...
		if do_val:
			...
			ave_slants, p_acc= predict_party_membership(tst_slants, Party_test_id, As_test)
		...
		assert (val_parties == parties_val).all(), 'party order changed'
		...
		Slantbyid_pred, _ = predict_party_membership(val_slants, Party_by_id, As_val)
		...
		##### by author framings
		authors = np.argmax(As_val, axis = -1)
		print(authors[:10])
		...
		np.save(FLAGS.resultdir + '/Slantbyid_pred_{}'.format(round), Slantbyid_pred)
...
if __name__ == '__main__':
	...
	if FLAGS.tuning:
		...
		# parameterdict = {"masks":[0.08, 0.05, 0.03, 0.01, 0.8, 0.6, 0.4,0.2, 0.1],  'epochs':[10]} # epochs >=5

		parameterdict = {"masks":[ 0.8, 0.6, 0.4,0.2, 0.1],  'epochs':[10]} # epochs >=5
		# parameterdict = {"masks":[ 0.1, 0.08, 0.05, 0.03, 0.01],  'epochs':[20]}
		# parameterdict = {"masks":[0.8],  'epochs':[1]} # epochs >=5
