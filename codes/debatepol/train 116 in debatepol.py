...
vs = 116# vamprior, adding masked labels, randomly masked
penalty = 1
rp = 2 ##
extreme = 5
...
def model(TrainD, TrainA,  TrainThe, TrainID, TrainParty, TrainMask,T0, T00, Zbar = None, updateT = None, trained = True, reuse = False):
	trainf = True
	traint = True 
	...
	with tf.variable_scope('variational_f', reuse = reuse):
		...
		z_mu_mean = tf.reduce_mean(z_mu, axis = -1)
	...
	if trained:
		...
		# SlantLoss = FLAGS.beta_s *tf.compat.v1.losses.sigmoid_cross_entropy(TrainParty, s_logit)
		# SlantLoss = FLAGS.beta_s * custom_crossentrophy(s_logit, TrainParty)
		...
		if updateT is None:
			...
			elbo = tf.reduce_mean(- mse -kl_mu  -  ThetaLoss - TmatrixLoss  -SlantLoss -  regloss, 0)
			# elbo = tf.reduce_mean(-mse  -SlantLoss -regloss, 0)
		...
		train_ops = [train_op,  theta_n, s_logit, z_mu_mean, SlantLoss, batchno.assign_add(1)] # edited by XR: delete f, change ft to f
...
def train(trains, tests, T0, T00, Zbar = None,  updateT = None, round = 0, do_val = False, writesummary = False):
	...
	with tf.Graph().as_default():
		...
		train_dataset = tf.data.Dataset.from_tensor_slices((Ds, As,  Thetas, Parties, Ids, Masks))
		# train_dataset = train_dataset.shuffle(buffer_size = 100000)
		train_dataset = train_dataset.batch(FLAGS.batch_size)

		train_iterator = train_dataset.make_initializable_iterator()
		TrainD, TrainA, TrainThe,  TrainParty,TrainID, TrainMask= train_iterator.get_next()

		tf.cast(TrainD, tf.float32)
		tf.cast(TrainA, tf.float32)


		test_dataset = tf.data.Dataset.from_tensor_slices((Ds_test,  As_test, Thetas_test,  Parties_test, Ids_test, Masks_test))
		test_dataset = test_dataset.batch(FLAGS.batch_size)
		test_iterator = test_dataset.make_initializable_iterator()
		TstD, TstA, TstThe,  TstParty,TstID, TstMask= test_iterator.get_next()
		# tf.cast(TrainD, tf.float32)
		# tf.cast(TrainA, tf.float32)
		

		train_ops= model(TrainD, TrainA,  TrainThe, TrainID, TrainParty, TrainMask,T0, T00, Zbar = Zbar, updateT = updateT)
		saver = tf.train.Saver()
		test_ops = model(TstD,  TrainA, TstThe, TstID, TstParty, TstMask,T0, T00, trained = False, reuse = True, updateT = None, Zbar = Zbar)
			...
			for epoch in range(FLAGS.n_epochs):
				...
				while True:
					try:
						...
						_, theta_t_, S_pred_, z_mu_mean,Slantloss, batchcount = sess.run(train_ops)  # Ds, As, Thetas, Parties,Ids
						trn_slants.extend(S_pred_)

						trn_thetas.extend(theta_t_)
						z_mu_means.extend(z_mu_mean)
						slantlosses.extend(Slantloss)
				...
				# print("slant, thetas, zmu means", trn_slants[:20], z_mu_means[:50])
				# print("mean of thetas 0 dim", np.array(trn_thetas).mean(axis  = 0))
				# print(np.array(trn_thetas).sum(axis = 1)[:20])
				print('slant means ', np.array(trn_slants).mean())
				print("mean of z_mu_means", np.array(z_mu_means).mean())
				print("mean of slant losses", np.array(slantlosses).mean())
...
def iterative_training():
	...
	for round in range(FLAGS.alter_rounds):
		...
		if do_val:
			tst_slants,  tst_thetas= train(trains, tests, T0= T0, T00=T00, Zbar= Zbar0, round = round, updateT= updateT, writesummary = False, do_val = do_val)
...
if __name__ == '__main__':
	...
	if FLAGS.tuning:
		...
		# parameterdict = {"masks":[0.6],  'epochs':[10]}
		...
		with open(oldresultp+'all_accuracy_{}_{}_repeat_{}.txt'.format(vs, bia,rp), 'w') as f:
			...
			f.write('\tslant_acc\tmembership_acc\ttopic_acc\n')
			...
			for msk, epc in parametergrid:
				...
				t_accs, s_accs, p_accs = iterative_training()
				...
				f.write('\n')
				print('^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^')
				print(versname, s_accs, t_accs, p_accs)
				print('^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^')
