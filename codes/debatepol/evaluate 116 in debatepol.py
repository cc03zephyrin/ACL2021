...
def train( tests, T0, T00, Zbar = None,  updateT = None, round = 0, do_val = False, writesummary = False):
	...
	with tf.Graph().as_default():
		...
		test_ops = model(TstD,  TstA, TstThe, TstID, TstParty, TstMask,T0, T00, trained = False, updateT = None, Zbar = Zbar)
...
def iterative_training():
	...
	for round in range(1):
		...
		updateT = None
		do_val = True
		test_id = np.arange(len(parties_all))
		...
		if do_val:
			tst_slants,  tst_thetas= train(tests, T0= T0, T00=T00, Zbar= Zbar0, round = round, updateT= updateT, writesummary = False, do_val = do_val)
			...
			s_accs.append(s_acc)
			p_accs.append(p_acc)
		print("topic acc, slant acc and party acc", t_accs, s_accs, p_accs)
			# print((tst_slants>0))
