...
def model(TrainD,  TrainA, TrainID, TrainParty, TrainMask, Zbar = None, trained = True, reuse = False):
	...
	with tf.name_scope('data'):
		...
		if Zbar is not None:
			...
			Zbar = tf.cast(tf.constant(np.random.normal(size = (FLAGS.n_authors, FLAGS.latent_dim))), tf.float32)
...
def train( tests, Zbar = None,  round = 0, do_val = False, writesummary = False):
	...
	with tf.Graph().as_default():
		...
		test_ops = model(TstD,TstA, TstID, TstParty, TstMask, trained = False, reuse = False,  Zbar = Zbar)
...
def iterative_training():
	...
	for round in range(1):
		...
		if do_val:
			tst_slants = train( tests, Zbar= Zbar0, round = round,  writesummary = False, do_val = do_val)
