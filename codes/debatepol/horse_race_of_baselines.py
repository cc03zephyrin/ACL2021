...
def bi_lstm_sample(data, pty, maxlen = 300, max_features = 50000):
...
def bi_lstm(X, y, train_id, test_id,  batch_size = 500, n_epoch = 10, max_features = 50000, trial = False): # 80% padding, 20% trancate
    if trial:
        num = -1000
        nu = -100
        n_epoch = 1
...
#     merged = Concatenate(axis = -1)([forwards, backwards])
    ...
    party_p = np.matmul(As_all[test_id][nu:].T[valid], logit)/np.sum(As_all[test_id][nu:].T[valid], axis = 1)>0
...
def experiments(data, pty,models_names = ['gru'], models_dict = {'gru': [bi_lstm_sample, bi_lstm]}, notmasks = [0.01, 0.03, 0.05, 0.08, 0.2, 0.4, 0.6, 0.8]):
    for md in models_names:
        for biased in [True, False]:
            ...
            if not biased:
                bia = 'unbias'
            f = open( r'D:/projects/congress/results/baselines/{}_{}_2.csv'.format(md, bia), 'w')
...
notmasks = [ 0.05, 0.08, 0.2, 0.4, 0.6, 0.8]
# notmasks = [ 0.6]
