...
vs = 100# vamprior, adding masked labels, randomly masked
penalty = 1
rp = 2 # 2 for biased smaple without replacement
...
flags.DEFINE_integer('n_epochs', 15, 'number of epochs') # default 30
...
flags.DEFINE_string('resultdir', r'D:\projects\congress\results\debatepol\test/debate_pred_v{}.{}/'.format(vs, rp), 'directory for results')
flags.DEFINE_string('modelpath', r'D:\projects\congress\models\debatepol/vae_semi_v{}_{}/'.format(vs, rp), 'Directory for loading models')
...
def model(TrainD,  TrainID, TrainParty,  trained = True, reuse = False):
	...
	if trained:
		...
		SlantLoss = FLAGS.beta_s * tf.losses.sigmoid_cross_entropy(multi_class_labels=TrainParty, logits=s_logit)# 
		...
		train_op = optimizer.minimize(SlantLoss+ regloss)
		# if writesummary:
...
def train( tests, round = 0, do_val = False, writesummary = False):
	...
	Ds_test, As_test,   Parties_test, Ids_test = tests
	...
	print(model_dir)
	model_name = f'/{mround}/vae_{mround}'
	...
	with tf.Graph().as_default():
		...
		test_dataset = tf.data.Dataset.from_tensor_slices((Ds_test,  As_test,   Parties_test, Ids_test))
		test_dataset = test_dataset.batch(FLAGS.batch_size)
		test_iterator = test_dataset.make_initializable_iterator()
		TstD, TstA,   TstParty,TstID= test_iterator.get_next()
		# tf.cast(TrainD, tf.float32)
		# tf.cast(TrainA, tf.float32)
		...
		test_ops = model(TstD, TstID, TstParty, trained = False, reuse = False)
...
def iterative_training():
	...
	ds_all = np.load(os.path.join(datapath, 'sentence_vectors_guns_wmoder.npy'))



	thetas_soft_all = np.load(os.path.join(datapath, 'topics_soft_guns_wmoder.npy'))

	thetas_hard_all = np.load(os.path.join(datapath, 'topics_hard_guns_wmoder.npy'))
	...
	parties_all = np.load(os.path.join(datapath, 'stance_lib_binary_guns_wmoder.npy'))
	stance_all = np.load(os.path.join(datapath, 'stance_lib_7scale_guns_wmoder.npy'))
	y = parties_all.copy()
	...
	# print(ds.shape)
	As_all = np.load(os.path.join(datapath, 'author_dummies_guns_wmoder.npy')) # change it to true authors when applicable



	ids_all = np.load(os.path.join(datapath, 'speech_ids_guns_wmoder.npy'), allow_pickle= True)
	...
	for round in range(1):
		...
		print('shape', ds_test.shape)
		# sort the input, non masked first

	
		tests = [ds_test, As_test,  parties_test, ids_test]
		if do_val:
			tst_slants = train(tests,  round = round,  writesummary = False, do_val = do_val)
...
if __name__ == '__main__':
	...
	if FLAGS.tuning:
		...
		for msk, epc in parametergrid:
			...
			slants_t = iterative_training()
			np.save(FLAGS.resultdir + 'predicted_slant_gun_topics_withmoder', slants_t)
